...
## Step 1: Planing
- WebSite:
    - On the first time a user connects to the WebSite, or if they return and have not acknoledged the website cookie policy
        - A popup that disables the use of the website until acknowledgement by the user needs to be displayed explaining the websites uses of cookies
            - Options must be provided for the visitor providing the standard options
                - /home/rogue/Data/data/coding/github/shopify/app/hoseworld/2607311135-website_cookies.md
            - The users choices must be stored for legalety and must be amendable by the user/customer/visitor
                - The users already have a profile page so maybe we can use that

## Step 2: Documentation and exit
- Update Verion to reflect the Commit count as per every task
- update @2607311150_todo.md but exclude todo.md from any updating
